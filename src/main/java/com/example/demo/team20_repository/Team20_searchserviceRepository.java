...
public interface Team20_searchserviceRepository extends JpaRepository<Team20_Shain,String>{
	@Query(value="select * from shain_tbl s "+
            ...
            "   or s.rank1 in (select h.hobbycode from hobby_tbl h where (coalesce(:janru, '') = '' or h.janru = :janru) and (coalesce(:hobby, '') = '' or h.hobby = :hobby)) " +
            "   or s.rank2 in (select h.hobbycode from hobby_tbl h where (coalesce(:janru, '') = '' or h.janru = :janru) and (coalesce(:hobby, '') = '' or h.hobby = :hobby)) " +
            "   or s.rank3 in (select h.hobbycode from hobby_tbl h where (coalesce(:janru, '') = '' or h.janru = :janru) and (coalesce(:hobby, '') = '' or h.hobby = :hobby))) "
				,nativeQuery=true)
