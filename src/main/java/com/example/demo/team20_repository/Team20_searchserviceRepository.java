...
public interface Team20_searchserviceRepository extends JpaRepository<Team20_Shain,String>{
	@Query(value="select * from shain_tbl s "+
				" where (:name is null or shainnm like '%'||:name||'%')"
				+"and (:job is null or job = :job)"
			+"and( (:janru is null and :hobby is null)"
				+"or s.rank1 =(select h.hobbycode from hobby_tbl h where h.janru =:janru and h.hobby= :hobby)"
			+"or s.rank2 =(select h.hobbycode from hobby_tbl h where h.janru =:janru and h.hobby= :hobby)"
				+"or s.rank3 =(select h.hobbycode from hobby_tbl h where h.janru =:janru and h.hobby= :hobby))"
				,nativeQuery=true)
