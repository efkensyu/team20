...
public interface Team20_searchserviceRepository extends JpaRepository<Team20_Shain,String>{
	@Query(value="select * from team20_shain_tbl s "+
			"where (coalesce(:loginUserid, '') = '' or s.shaincd != :loginUserid) " +
			"and (coalesce(:name, '') = '' or s.shainnm like concat('%',:name,'%')) " +
	...
	String hobby,@Param("job")String job,@Param("loginUserid") String loginUserid);
}